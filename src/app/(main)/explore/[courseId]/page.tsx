...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: course ? `${course.title} | پلتفرم یادگیری` : 'دوره',
...
export default async function CourseDetailPage({ params }: Props) {
  ...
  const courseSchema = {
    ...
    '@type': 'Course',
    name: course.title,
    url: `${process.env.NEXT_PUBLIC_SITE_URL}/explore/${courseId}`,
    inLanguage: 'fa',
  ...
  return (
    <>
      ...
      <CourseDetailClient courseId={courseId} courseTitle={course.title} />
    </>
  );
}
